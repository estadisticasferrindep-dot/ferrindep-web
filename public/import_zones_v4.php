<?php
// DB Zone Importer V4 - Explicit Assignment Edition
...
echo "🚀 INICIANDO IMPORTACION DE ZONAS (V4 - EXPLICIT)\n";
echo "=======================================\n\n";
...
foreach ($zonesToImport as $item) {
    ...
    try {
        ...
        if ($existing) {
            ...
            // 2. Create Destino (EXPLICIT ASSIGNMENT to bypass Guarded issues)
            echo "➕ CREANDO: '$name' ... ";

            $newDestino = new Destino();
            $newDestino->nombre = $name;
            $newDestino->zona_id = $zonaId; // Force set
            $newDestino->save();

            if ($newDestino->id) {
                // 3. Link to Zona (Pivot)
                try {
                    $pivot = new Destinozona();
                    $pivot->destino_id = $newDestino->id;
                    $pivot->zona_id = $zonaId;
                    $pivot->save();
                } catch (\Exception $e) {
                    echo "(Pivot Warning) ";
                }
