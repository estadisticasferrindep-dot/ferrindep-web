            ...
            <div class="d-block d-md-none mobile-cards-container">
                <div v-for="(presentacion, key) in presentaciones2" :key="'mob-'+key" v-if="presentacion.show" class="mobile-card" style="flex-direction: column;">
                    <div class="d-flex w-100 justify-content-between">
                        <!-- Left Thumbnail -->
                        <div v-if="imagen" class="mobile-thumb-container" style="flex: 0 0 65px; margin-right: 12px; display:flex; align-items:flex-start; justify-content:center;">
                            <img :src="imagen" :alt="presentacion.nombre" style="width: 100%; height: auto; object-fit: contain; border-radius: 4px;">
                        </div>

                        <div class="card-left" style="flex: 1; min-width: 0; padding-right: 8px; display: flex; flex-direction: column; justify-content: center;">
                            <!-- Medidas mobile -->
                            <!-- Medidas mobile -->
                            <div style="font-size: 10px; color: #777; margin-bottom: 2px; letter-spacing: -0.3px; line-height: 1.2; white-space: normal !important; overflow: visible !important; height: auto;">
                        ...
                        <div class="card-right" style="flex: 0 0 auto; display: flex; flex-direction: column; align-items: flex-end; justify-content: center;">
                            ...
                            <div v-else-if="(presentacion.stock == 1 )" class="last-unit">
                                ...
                                <div class="stepper-control">
                                    <button v-if="cantidades[presentacion.id] == 0" class="btn-agregar-mob" style="padding: 4px 8px; font-size: 10px; min-width: auto;" @click="increment(presentacion)">
                                        AGREGAR
                                    </button>
                            ...
                            <div v-else class="stepper-control">
                                <button v-if="!cantidades[presentacion.id] || cantidades[presentacion.id] == 0" class="btn-agregar-mob" style="padding: 4px 8px; font-size: 10px; min-width: auto;" @click="increment(presentacion)">
                                    AGREGAR
                                </button>
                                <div v-else class="stepper-ui">
                                    <button class="step-btn minus" @click="decrement(presentacion)">-</button>
                    ...
                    <div v-if="getPresentationShipping(presentacion)" style="width: 100%; font-size: 10px; margin-top: 6px; padding-top: 4px; border-top: 1px solid #eee; text-align: left;">
                        ...
                        <span v-else-if="getPresentationShipping(presentacion).type === 'cost'" style="color: #FD914D; font-weight: bold;">
                            <i class="fas fa-truck" style="font-size: 9px;"></i> {{ getPresentationShipping(presentacion).text }}
                        </span>
                        <span v-else-if="getPresentationShipping(presentacion).type === 'unknown'" style="color: #17a2b8; font-weight: bold;">
                            <i class="fas fa-map-marker-alt" style="font-size: 9px;"></i> {{ getPresentationShipping(presentacion).text }}
                        </span>
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        computed: {
            ...
            formattedLocationName() {
                // Strategy: Rely on String checks mostly to respect "Villa Ballester" etc.
                // We do NOT force CABA just because it's Zone 1, as Zone 1 might include GBA.
                ...
                // Common API alias fallback
                if (this.locationName === "Buenos Aires") return "CABA";
            ...
            dynamicShippingLabel() {
                ...
                // 2. PRIORITY: Use server-resolved Flex shippingRates
                let zonePrice = 0;
                let zoneId = null;
                let usedFlex = false;

                if (this.shippingRates && this.shippingRates.length > 0 && parseFloat(this.shippingRates[0].costo || 0) > 0) {
                    zonePrice = parseFloat(this.shippingRates[0].costo);
                    zoneId = this.zonaId || 'flex';
                    usedFlex = true;
                } else {
                    // FALLBACK: Client-Side Legacy Resolution (only if Flex not available)
                    let resolved = this.resolveClientSideZone();
                    if (resolved) {
                        zoneId = resolved.id;
                        zonePrice = resolved.price;
                    } else if (this.realZonaId && this.realZonaId != 'null') {
                ...
                for (let i = 0; i < this.presentaciones2.length; i++) {
                     ...
                     if (qty > 0) {
                        hasSelection = true;
                        let weight = parseFloat(p.peso || 0);
                        totalWeight += (weight * qty);

                        let isFreeItem = false;
                        if (usedFlex) {
                            // Flex free shipping check
                            if (p.envio_gratis_flex == 1 || p.envio_gratis == 1) isFreeItem = true;
                        } else {
                            // Legacy free shipping check
                            ...
                            if (p.envio_gratis == 1) isFreeItem = true;
                        }
                ...
                // SCENARIO B: Paid Shipping (Apply 30kg Rule)
                ...
                return `<div>
                            <i class="fas fa-truck"></i> Envío a ${this.formattedLocationName} desde <b>$${formattedCost}</b>
                        </div>`;
            }
        },
        methods: {
...
// console.log('add'+ this.anulaEnvio)
            ...
            getPresentationShipping(p) {
                // 0. Check Pickup
                if (this.isPickupOnly) {
                    return { type: 'cost', text: 'Por el momento, solo retiro por nuestro deposito' };
                }

                // 1. PRIORITY: Use server-resolved Flex shippingRates
                let baseCost = 0;
                let zoneId = null;

                if (this.shippingRates && this.shippingRates.length > 0 && parseFloat(this.shippingRates[0].costo || 0) > 0) {
                    baseCost = parseFloat(this.shippingRates[0].costo);
                    zoneId = this.zonaId || 'flex';
                } else {
                    // FALLBACK: Client-Side Legacy Resolution
                    let resolved = this.resolveClientSideZone();
                    zoneId = resolved ? resolved.id : (this.realZonaId || this.zonaId);
                    
                    if (resolved && (resolved.precio || resolved.costo || resolved.price)) {
                        baseCost = parseFloat(resolved.precio || resolved.costo || resolved.price || 0);
                    }
                }
                
                // If NO Zone and NO Location Name -> Return Unknown/Prompt
                if (!zoneId) {
                    if (this.formattedLocationName) {
                        return { type: 'unknown', text: 'Por el momento no realizamos envíos a esta ubicación' };
                    } else {
                        return { type: 'unknown', text: 'Ingresá tu ubicación para ver costos' };
                    }
                }
                ...
                // 2. Determine Free Shipping
                let isFree = false;
                if (p.envio_gratis_flex == 1 || p.envio_gratis == 1) isFree = true;

                let destName = this.formattedLocationName || 'su ubicación';
                
                if (isFree) {
                    return { type: 'free', text: 'Envío sin cargo a ' + destName };
                }

                // 3. Calculate Weight-based Cost (30kg rule)
                let pesoUnitario = parseFloat(p.peso || 0);
                let qty = (this.cantidades && this.cantidades[p.id]) ? parseInt(this.cantidades[p.id]) : 1;
                if (qty < 1) qty = 1;

                let totalWeight = pesoUnitario * qty;
                let bultos = Math.max(1, Math.ceil(totalWeight / 30));
                
                let finalCost = baseCost * bultos;

                return { type: 'cost', text: 'Entrega a domicilio en ' + destName + ' $' + this.formatPrice(finalCost) };
            },

            getShippingLabel(producto) {
                // Re-use logic or implement similar robust logic
                let info = this.getPresentationShipping(producto);
                if (!info) return null;
                
                // Keep the HTML format for Desktop Table
                if (info.type === 'free') {
                    return `<div style="font-size:11px; margin-top:0px; margin-bottom:2px; color:#28a745; font-weight:bold; line-height:1.1;">
                                <i class="fas fa-truck"></i> ${info.text}
                            </div>`;
                }

                // UNKNOWN / PROMPT
                if (info.type === 'unknown') {
                     return `<div style="font-size:11px; margin-top:0px; margin-bottom:2px; color:#17a2b8; font-weight:bold; line-height:1.1;">
                                <i class="fas fa-map-marker-alt"></i> ${info.text}
                            </div>`;
                }

                // For cost/pickup - RESTORE ORANGE COLOR
