                  ...
                  <div>
                      ...
                      <div id="cortador-virtual-app" class="p-3 border rounded bg-light">
                          ...
                          <div id="lista-acumulada" class="mt-3 d-none">
                              ...
                              <div id="debug-log" class="alert alert-info small" style="display:block; word-break:break-all;">DEBUG MODE ON</div>
                              <button id="btn-enviar-carrito-manual" class="btn btn-primary w-100 py-2 fw-bold" style="background:#FD914D; border:none;">
                          ...
                          <div class="mt-4 pt-3 border-top">
                                <h6 style="color:#FD914D; font-weight:bold; margin-bottom:10px;"><i class="fas fa-truck"></i> Calculá tu envío</h6>
                                <div class="mb-2">
                                    <label for="selectDestino" class="form-label small text-muted">Seleccioná tu localidad:</label>
                                    <select id="selectDestino" class="form-select form-select-sm" style="font-size:13px;">
                                        <option value="">Cargando...</option>
                                    </select>
                                </div>
                                <div class="d-grid">
                                    <button id="btnCalcularEnvio" class="btn btn-sm btn-outline-secondary" type="button">CALCULAR COSTO</button>
                                </div>
                                <div id="resultadoEnvio" class="mt-2 text-center small" style="min-height:20px;"></div>
                          </div>
                      </div>
              ...
              @else
                  ...
                  <div>
                    ...
                    <add-to-cart 
                        ...
                        :medidas="{{ json_encode($producto->medidas->medidas) }}" 
                        :espesor="{{ json_encode($producto->espesor->espesor) }}" 
                        :ancho="{{ json_encode($producto->categoria->nombre) }}" 
                        :familia="{{ json_encode($producto->familia->nombre) }}" 
                        :con-nombre="{{ $producto->con_nombre }}" 
                        :vendidos="{{ $producto->vendidos }}" 
                        :anula-envio="{{ $producto->anular_envio }}" 
                        :presentaciones="{{ json_encode($producto->presentaciones) }}" 
                        :imagen="{{ json_encode($principalUrl) }}" 
                        :id="{{ $producto->id }}" 
                        :oferta="{{ $producto->oferta }}" 
                        :ruta="{{ json_encode('/carrito') }}" 
                        ref="me" 
                    />
                  </div>
              @endif
...
<script>
    ...
    // Asignar evento al botón de forma segura
    const btnEnviar = document.querySelector('button[onclick="enviarAlCarrito()"]');
    if(btnEnviar) {
        btnEnviar.removeAttribute('onclick'); // Quitar inline para evitar conflictos
        btnEnviar.addEventListener('click', function(e) {
            e.preventDefault();
            logDebug("Botón clickeado...");
            enviarAlCarrito();
        });
    } else {
        const btnManual = document.getElementById('btn-enviar-carrito-manual');
        if(btnManual) {
            btnManual.addEventListener('click', function(e){
                e.preventDefault();
                logDebug("Botón Manual clickeado...");
                enviarAlCarrito();
            });
        }
    }
    ...
    window.enviarAlCarrito = function() {
        logDebug("Iniciando enviarAlCarrito...");
        try {
            if(carritoTemporal.length === 0) { 
                alert("Suma al menos un rollo."); 
                logDebug("Carrito vacío.");
                return; 
            }
            
            logDebug("Leyendo LocalStorage...");
            let cart = [];
            try {
                const raw = localStorage.getItem('cartQunuy');
                logDebug("Raw LS: " + (raw ? raw.length + " chars" : "null"));
                cart = JSON.parse(raw) || [];
            } catch (e) {
                logDebug("Error JSON parse: " + e.message);
                console.error("Error parsing cart:", e);
                cart = [];
            }
            logDebug("Items actuales en cart: " + cart.length);
            ...
            carritoTemporal.forEach(itemLista => {
                ...
                if (existente) { existente.cantidad = parseInt(existente.cantidad) + 1; }
                    ...
                    cart.push({
                        ...
                        descripcion: {!! json_encode($producto->descripcion) !!}, imagen: "{{ $principalUrl }}",
                        anulaEnvio: {{ $producto->anular_envio ? 'true' : 'false' }}, familia: "{{ $producto->familia->nombre }}"
                    });
                }
            });
            
            logDebug("Guardando LocalStorage...");
            localStorage.setItem("cartQunuy", JSON.stringify(cart));
            
            const urlDestino = "/carrito";
            logDebug("Redirigiendo a: " + urlDestino);
